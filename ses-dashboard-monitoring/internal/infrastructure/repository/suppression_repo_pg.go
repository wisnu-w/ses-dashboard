...
func (r *suppressionRepo) Add(ctx context.Context, entry *suppression.SuppressionEntry) error {
	query := `
		INSERT INTO suppressions (email, suppression_type, reason, aws_status, is_active, added_by, created_at, updated_at)
...
func (r *suppressionRepo) MarkAsSynced(ctx context.Context, email string) error {
	query := `UPDATE suppressions SET synced_at = NOW() WHERE email = $1`
	_, err := r.db.ExecContext(ctx, query, email)
	return err
}
